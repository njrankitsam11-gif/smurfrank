...
const HotDealsFeed = React.memo(function HotDealsFeed() {
  ...
  // 💡 What: Extracted static `feedItems` out of the component and wrapped `HotDealsFeed` in `React.memo()`.
  // 🎯 Why: This component sits on the HomePage alongside the `useCart` hook. Without memoization, every cart update forces this complex Framer Motion component to re-render, creating unnecessary DOM diffing and potentially restarting animations.
  // 📊 Impact: Eliminates re-renders of the animated feed on unrelated parent state changes (like adding items to cart), improving rendering performance and CPU usage.

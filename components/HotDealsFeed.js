...
// 💡 What: Wrapped `HotDealsFeed` in `React.memo()` and moved `feedItems` outside the component.
// 🎯 Why: `HotDealsFeed` is a completely static, prop-less animated component (Framer Motion feed/ticker). Without memoization, every parent update triggers a full re-render, unnecessarily restarting the animation and creating DOM diffing overhead. Moving `feedItems` outside prevents redefining the array on every render.
// 📊 Impact: Eliminates unnecessary re-renders of the animated ticker, preventing animation stutters and reducing React render cycle overhead.

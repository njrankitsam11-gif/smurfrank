...
// ⚡ BOLT OPTIMIZATION: Memoize prop-less animated component and extract static array
// 💡 What: Moved `feedItems` outside the component and wrapped `HotDealsFeed` in `React.memo`.
// 🎯 Why: This prop-less animated feed is nested inside a parent that might re-render, which would cause unnecessary DOM diffing and animation restarts.
// 📊 Impact: Prevents re-renders of the animated feed and reduces garbage collection by ensuring the array has a stable reference.
const feedItems = [
...
export default React.memo(HotDealsFeed);

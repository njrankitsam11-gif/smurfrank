...
// 💡 What: Moved `feedItems` out of the component scope and wrapped `HotDealsFeed` in `React.memo()`.
// 🎯 Why: This component has no props and completely static data, but runs a continuous Framer Motion animation. When the parent component re-renders (e.g., when a user adds something to the cart), it forces this component to re-render, destroying the array reference and potentially causing layout trashing or animation restarts.
// 📊 Impact: Prevents unnecessary DOM diffing and avoids resetting the 25-second continuous scrolling animation on global state changes.

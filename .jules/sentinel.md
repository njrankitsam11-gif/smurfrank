...
**Vulnerability:** User emails were queried using `findUnique` in `app/api/register/route.js` and `app/api/auth/[...nextauth]/route.js`, which does not support case-insensitive matching in Prisma.
**Learning:** `findUnique` does not support `mode: 'insensitive'`. To perform case-insensitive queries (e.g., on emails), use `findFirst`.
**Prevention:** Always use `findFirst` with `mode: 'insensitive'` and normalize emails (`trim().toLowerCase()`) during registration and authentication.

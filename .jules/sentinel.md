...
## 2026-04-14 - [Case-Sensitive Email Lookup Vulnerability]
**Vulnerability:** Registration and authentication endpoints used Prisma's `findUnique` for email lookups, which is case-sensitive by default. This allowed registering duplicate accounts (e.g., `User@example.com` and `user@example.com`), leading to potential account takeovers, confusion, or evasion of bans and rate limits.
**Learning:** While `trim().toLowerCase()` helps with rate-limiting keys, relying on case-sensitive database lookups for uniquely identifying users is insecure. Prisma's `findUnique` does not natively support case-insensitive querying.
**Prevention:** Always normalize email inputs before processing. For database queries in Prisma where case insensitivity is required (like email addresses), replace `findUnique` with `findFirst` and use `mode: 'insensitive'`. Note: Ensure all test mocks are also updated from `findUnique` to `findFirst` to prevent test suite failures.

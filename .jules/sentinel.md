...
## 2026-04-19 - [Email Case-Sensitivity Vulnerability]
**Vulnerability:** Case-sensitive email handling during registration and authentication could allow duplicate account creation (e.g., `Admin@example.com` vs `admin@example.com`), leading to account confusion or takeover.
**Learning:** Prisma's `findUnique` does not support `mode: 'insensitive'` queries for strings. It must be replaced by `findFirst` with the `{ equals: email, mode: 'insensitive' }` object syntax.
**Prevention:** Always normalize email inputs (`trim().toLowerCase()`) and perform case-insensitive queries when checking for existing users or authenticating.

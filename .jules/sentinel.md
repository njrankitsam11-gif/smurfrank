...
## 2026-04-07 - [Missing Content Security Policy]
...
**Learning:** Next.js applications require an explicit `Content-Security-Policy` header in `next.config.mjs` to enforce strict resource loading policies.
**Prevention:** Always include a comprehensive CSP that limits `img-src` and `script-src` to known, trusted domains and prevents `unsafe-inline` scripts where possible.

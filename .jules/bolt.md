...
**Learning:** Performing expensive operations like regex matching and string parsing inside a sort comparator function results in O(N log N) overhead, which can be significant for larger datasets. Pre-calculating these values once (O(N)) before sorting reduces the comparator to simple numeric subtraction.
**Action:** Always pre-calculate or memoize complex sort keys before invoking .sort() to ensure the comparator remains O(1) and the overall sort operation stays efficient.
## 2026-04-18 - Memoizing Static Components to Avoid Re-renders\n**Learning:** In React, defining a static array inside a functional component causes it to be recreated on every render. Passing this to a child component (or rendering it directly in a way that depends on reference equality) can trigger unnecessary re-renders or DOM diffing, especially when wrapped in animations (like Framer Motion).\n**Action:** Always move static arrays and objects outside of the component body to maintain a stable reference across renders. For components that do not rely on props and should not re-render when the parent does, wrap them in `React.memo()` to bypass reconciliation entirely.

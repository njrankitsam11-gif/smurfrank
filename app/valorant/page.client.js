...
export default function ValorantPage() {
  ...
  // ⚡ BOLT OPTIMIZATION: Pre-parse sorting keys (Schwartzian Transform) and memoize sorting logic
  // 💡 What: Mapped `price` strings to `numericPrice` numbers once before sorting, wrapped in useMemo, and hoisted static `products` array outside component.
  // 🎯 Why: Parsing floats and running regex inside a sort comparator creates an O(N log N) overhead, and recalculating parsing logic on every render is inefficient.
  // 📊 Impact: Reduces expensive regex string manipulation from O(N log N) to O(N), prevents parsing on unrelated renders, and stops the static `products` array from being recreated on every render, significantly improving execution time.

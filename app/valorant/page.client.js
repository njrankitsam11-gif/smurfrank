...
export default function ValorantPage() {
  ...
  // ⚡ BOLT OPTIMIZATION: Pre-parse sorting keys & Memoization (Schwartzian Transform)
  // 💡 What: Hoisted static `products` array and used `useProductSort` to memoize the mapped numeric sorting keys.
  // 🎯 Why: Parsing prices and running sorts on every render is inefficient (O(N log N)). Un-memoized, inline arrays also cause unnecessary garbage collection and break referential equality.
  // 📊 Impact: O(N log N) -> O(N) sorting efficiency, preventing expensive recalculations and re-renders on unrelated state changes.

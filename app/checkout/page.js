  ...
  return (
    <main style={s.page}>
      ...
      <div style={{ textAlign:'center', marginBottom:48 }}>
        ...
        <h1 style={{ fontSize:36, fontWeight:900, letterSpacing:'-1.5px', margin:0 }}>
          Complete Your <span style={{ color:'#FF6A00' }}>Order</span>
        </h1>
      ...
      <div style={s.grid} className="checkout-grid">
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:12, marginTop:16 }}>
              {PAYMENT_METHODS.map(m => (
                <label key={m.id} style={{
                  ...
                  padding:'14px 18px',
                  border:`1px solid ${selectedMethod===m.id ? '#FF6A00' : '#1f1f2e'}`,
                  borderRadius:8, cursor:'pointer',
                  background: selectedMethod===m.id ? 'rgba(255,106,0,0.06)' : '#0f0f17',
                  transition:'all 0.2s',
                }}>
                  <input
                    ...
                    style={{ accentColor:'#FF6A00', width:18, height:18 }}
                  />
                  <span style={{ fontSize:22 }}>{m.icon}</span>
                  <span style={{ fontWeight:700, fontSize:14 }}>{m.name}</span>
                  {selectedMethod===m.id && (
                    <span style={{ marginLeft:'auto', color:'#FF6A00', fontSize:11, fontWeight:900 }}>SELECTED</span>
                  )}
                </label>
              ))}
            </div>
          ...
          <div style={s.card}>
            ...
            <p style={{ color:'#555', fontSize:12, fontStyle:'italic', margin:'12px 0 20px' }}>
              API integration for <strong style={{ color:'#888' }}>{PAYMENT_METHODS.find(m=>m.id===selectedMethod)?.name}</strong> will be configured with your keys.
            </p>
              ...
              <form onSubmit={handlePayment} style={{ display:'flex', flexDirection:'column', gap:16 }}>
                ...
                <div style={s.cardPreview}>
                  <div style={{ display:'flex', justifyContent:'space-between' }}>
                    <span style={{ fontWeight:900, fontSize:13, letterSpacing:2, color:'#66FCF1' }}>SMURFRANK</span>
                    <span style={{ fontSize:22 }}>💳</span>
                  </div>
                  <div style={{ fontFamily:'monospace', fontSize:18, letterSpacing:4, color:'#fff', marginTop:18 }}>
                    {cardNum || '•••• •••• •••• ••••'}
                  </div>
                  <div style={{ display:'flex', justifyContent:'space-between', marginTop:14 }}>
                    <div>
                      <div style={{ fontSize:9, color:'#555', letterSpacing:2 }}>CARD HOLDER</div>
                      <div style={{ fontSize:13, fontWeight:800, color:'#ccc' }}>{nameOnCard || 'YOUR NAME'}</div>
                    </div>
                    <div style={{ textAlign:'right' }}>
                      <div style={{ fontSize:9, color:'#555', letterSpacing:2 }}>EXPIRES</div>
                      <div style={{ fontSize:13, fontWeight:800, color:'#ccc' }}>{expiry || 'MM/YY'}</div>
                    </div>
                  </div>
                </div>
            ...
            {['paypal','stripe','razorpay'].includes(selectedMethod) && (
              <form onSubmit={handlePayment}>
                <div style={{ textAlign:'center', padding:'20px 0 28px' }}>
                  ...
                  <p style={{ color:'#777', lineHeight:1.7, marginBottom:20, fontSize:14 }}>
                    You'll be redirected to the <strong style={{ color:'#ccc' }}>
            ...
            {selectedMethod === 'crypto' && (
              <form onSubmit={handlePayment} style={{ display:'flex', flexDirection:'column', gap:16 }}>
                ...
                <div style={{ background:'#0f0f17', border:'1px solid #1a1a2e', borderRadius:10, padding:18 }}>
                  <p style={{ color:'#555', fontSize:11, letterSpacing:2, margin:'0 0 10px', fontWeight:800 }}>WALLET ADDRESS</p>
                  <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                    <code style={{ color:'#66FCF1', fontSize:12, wordBreak:'break-all', flex:1 }}>
                ...
                <div style={{ background:'rgba(102,252,241,0.04)', border:'1px dashed #66FCF1', borderRadius:8, padding:'14px 18px', fontSize:12, color:'#66FCF1', lineHeight:1.6 }}>
                  ⚠️ Send <strong>${grandTotal.toFixed(2)} USD</strong> worth of {cryptoCoin}. Orders confirmed after 3 network confirmations.
                </div>
                <PayBtn isProcessing={isProcessing} total={grandTotal} label="I've Sent the Payment" color="#66FCF1" textColor="#000" />
              </form>
            )}
          </div>
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
            ...
            {cart.length > 0 && (
              <div style={{ marginTop:20 }}>
                ...
                <div style={{ borderTop:'1px solid #222', marginTop:16, paddingTop:16 }}>
                  <PriceLine label="TOTAL" val={`$${grandTotal.toFixed(2)}`} bold color="#FF6A00" />
                </div>
              </div>
            )}
          </div>
          ...
          <div style={s.card}>
            {[
              ...
              <div key={b.title} style={{ display:'flex', gap:14, alignItems:'flex-start', marginBottom:16 }}>
                ...
                <div>
                  <p style={{ margin:0, fontWeight:900, fontSize:12, color:'#ccc' }}>{b.title}</p>
                  <p style={{ margin:'4px 0 0', fontSize:11, color:'#555', lineHeight:1.5 }}>{b.desc}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* LOYALTY REWARD */}
          <div style={{ background:'rgba(102,252,241,0.04)', border:'1px dashed #66FCF1', borderRadius:10, padding:'18px 20px' }}>
            <p style={{ color:'#66FCF1', fontWeight:900, margin:'0 0 6px', fontSize:12 }}>🔥 LOYALTY REWARD</p>
            <p style={{ color:'#777', margin:0, fontSize:11, lineHeight:1.5 }}>
              Complete this order and get an <strong style={{ color:'#fff' }}>11% OFF coupon</strong> for your next purchase — sent instantly to your inbox.
            </p>
          </div>
        </div>
      </div>
...
function PriceLine({ label, val, dim, bold, color }) {
  return (
    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:10 }}>
      <span style={{ fontSize:bold?14:12, fontWeight:bold?900:700, color:dim?'#444':'#777' }}>{label}</span>
      <span style={{ fontSize:bold?20:13, fontWeight:900, color:color||'#ccc' }}>{val}</span>
    </div>
  );
}
...
const s = {
  page: {
    minHeight:'100vh',
    background:'linear-gradient(rgba(5,5,7,0.88), rgba(5,5,7,0.96)), url("https://images.unsplash.com/photo-1614680376573-df3480f0c6ff?q=80&w=1974&auto=format&fit=crop")',

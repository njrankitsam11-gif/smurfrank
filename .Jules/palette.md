...
## 2026-04-10 - Search Landmark Accessibility
**Learning:** Even if a search form has inputs with `aria-label`, the `<form>` element itself needs `role="search"` to be exposed as a distinct search landmark in the accessibility tree for screen readers.
**Action:** Always add an explicit `role="search"` attribute to the `<form>` element of any search forms.

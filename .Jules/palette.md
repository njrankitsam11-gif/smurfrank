...
## 2026-04-16 - Search Form Accessibility
**Learning:** Even if individual input fields have `aria-label`s, search forms need an explicit `role="search"` to be fully exposed as search landmarks in the accessibility tree, allowing screen readers to easily locate the primary search functionality.
**Action:** Always add `role="search"` to `<form>` elements that perform searches, and ensure the primary input uses `type="search"`.

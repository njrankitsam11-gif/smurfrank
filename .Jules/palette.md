...
## 2026-04-23 - Search Form Accessibility and Mobile UX
**Learning:** Search forms without an explicit `role="search"` are not reliably exposed as search landmarks in the accessibility tree, even if inner inputs have ARIA labels. Additionally, omitting `type="search"` on the input prevents mobile keyboards from displaying the optimal 'Search' action key.
**Action:** Always add `role="search"` to `<form>` elements used for searching, and ensure the corresponding input uses `type="search"` rather than defaulting to generic text.

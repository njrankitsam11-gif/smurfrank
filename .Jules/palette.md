...
## 2026-04-25 - Search Form Landmark and Semantics
**Learning:** Generic text inputs for search prevent mobile devices from showing the correct 'Search' keyboard layout, and forms without `role="search"` aren't exposed as proper search landmarks in the accessibility tree.
**Action:** Always use `type="search"` on search inputs and explicitly add `role="search"` to the parent `<form>` element to maximize accessibility and usability.

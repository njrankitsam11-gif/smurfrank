...
## 2026-04-28 - Search Landmark and Type Accessibility
**Learning:** Custom search forms sometimes rely purely on `aria-label`s on their inputs but miss the explicit `role="search"` on the parent form container, making it harder for screen readers to identify the search region. Additionally, generic text inputs don't trigger optimized search keyboard layouts on mobile devices.
**Action:** Always verify search forms have an explicit `role="search"` applied to the `<form>` and ensure the input field uses `type="search"` to provide appropriate semantic behavior and keyboard layouts.

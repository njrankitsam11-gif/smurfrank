...
A full-codebase audit was run before further feature work: `npm run lint`, `npx jest`, `npm run build`, and a manual scan for schema/field mismatches, dead code, and missing env docs. Result: **lint clean (0 errors), Jest 4/4 suites & 21/21 tests pass, production build succeeds.** Bun suite (`app/**/*.bun.test.js`, `context/CartContext.bun.test.js`, `components/CartDrawer.bun.test.js`) could not be run in this environment (no `bun` binary available) — verify it separately before treating the whole test suite as green. Three real bugs/gaps were found and fixed in this pass (see "Known loose ends" below for detail): a corrupted `package-lock.json` masking module-resolution errors, `app/sitemap.js` querying a non-existent `status` field (silently dropping every listing URL from the sitemap), and a missing `.env.example`/README env-setup section/`prisma.seed` config. Remaining known gaps (unchanged, deliberate, not bugs): checkout is UI-only/no real payment processor, RTK Query's `getListings` endpoint is dead code, cart state isn't unified with Redux, rate limiting is in-memory/per-process, and the root-level `patch*.js` files are unverified stray artifacts.
